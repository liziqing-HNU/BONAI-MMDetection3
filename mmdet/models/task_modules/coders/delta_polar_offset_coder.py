...
class DeltaPolarOffsetCoder(BaseBBoxCoder):
    """Encode roof-to-footprint offsets in polar coordinates."""
...
def offset2delta(proposals: Tensor,
                 ...
                 stds: Sequence[float] = (0.5, 0.5),
                 with_bbox: bool = True) -> Tensor:
    ...
    delta_length = gt[..., 0] / proposal_length if with_bbox else gt[..., 0]
    deltas = torch.stack([delta_length, gt[..., 1]], dim=-1)
...
def delta2offset(rois: Tensor,
    ...
    proposal_w = (rois[:, 2] - rois[:, 0]).unsqueeze(1).expand_as(
        delta_length)
    proposal_h = (rois[:, 3] - rois[:, 1]).unsqueeze(1).expand_as(
        delta_angle)
    proposal_length = torch.sqrt(proposal_w**2 + proposal_h**2)

    gt_length = proposal_length * delta_length if with_bbox else delta_length
    gt_angle = delta_angle
    if max_shape is not None:
        max_h, max_w = max_shape[:2]
        gt_length = gt_length.clamp(min=-max_w, max=max_w)
        gt_angle = gt_angle.clamp(min=-max_h, max=max_h)

    return torch.stack([gt_length, gt_angle], dim=-1).view_as(deltas)
